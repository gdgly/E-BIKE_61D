...
static void kfd_datetime_offset(applib_time_struct* datetime, S16 offset)
{
	...
	//×¢ÒâÊ±ÇøµÄÕý¸º
	if( (kal_uint16)offset < 32768)//ÕýÊý
	{
 ...
 * DESCRIPTION
 *  ×é°ü
 ÆðÊ¼·û2£¬Ð£ÑéÂë2£¬ÄÚÈÝ³¤¶È1£¬Ð­ÒéÀàÐÍ1£¬ÐòÁÐºÅ2£¬	ÈÕÆÚÊ±¼ä6£¬
 ÄÚÈÝcontext_len,	½áÊø·û2
 * PARAMETERS
 *  void  
 *  
 * RETURNS
 ...
 * DESCRIPTION
 * »ñÈ¡×î½üÒ»¸öÉÏ´«ÖÜÆÚµÄGPSÊý¾Ý×îÐ¡hdopµÄgpsÊý¾Ý
 * PARAMETERS
 *
 * RETURNS
 * gps_info_struct* 
 ...
 * DESCRIPTION
 * ×ª»»GPSÔ­Ê¼Êý¾ÝÎªkfdÐ­Òé¸ñÊ½Êý¾Ý
 * PARAMETERS
 *
 * RETURNS
 * kal_bool
...
kal_bool kfd_convert_gps_data_for_protocol(gps_info_struct* gps_data, gps_tracker_gps_struct* kfd_gps_data)
{
	...
	kfd_gps_data->speed = gps_data->speed*1.852*100;	//½Ú×ª»»Îª¹«Àï/Ð¡Ê±ÔÙ³ËÒÔ100
	...
	kfd_gps_data->msl_altitude = gps_data->altitude*10;	//·ÖÃ×
 ...
 * DESCRIPTION
 * dev_cfg  ÅäÖÃÏìÓ¦µÄ´¦Àíº¯Êý£¬¸ù¾Ý²»Í¬µÄÏûÏ¢ÀàÐÍ»Ø¸´²»Í¬µÄÏìÓ¦
 * PARAMETERS
 * RETURNS
 *  void
 *****************************************************************************/
			...
			else
			{
				...
				switch(control_data->addr)
				{
					...
					case 0x1c:	
					{
						...
						if(control_data->value[0]==0x02)	//»¹³µÖ¸Áî
						{
							kfd_upload_gps_package();
							kfd_upload_ebike_package();
						}
...
void kfd_protocol_parse(kal_uint8* rcv_buf,kal_int32 buflen)
{
	...
	for(i = 0; i<len-1; i++)
	{
		...
		else if(rcv_buf[i]==0x0d && rcv_buf[i+1]==0x0a)
		{
			...
			//¼ÌÐøÕÒ½áÎ²
			i +=1;	//´úÂë¼Ó1£¬forÑ­»·×Ô¼Ó1 £¬µÈÐ§ÓÚÏòºóÆ«ÒÆ2¸ö×Ö½Ú£¨0d0a£©			
		}
	}
